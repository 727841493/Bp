        ...
        public void Upload(FormCollection form)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    str.Append("[基本信息]" + "\r\n");
                    str.Append("上传文件名=" + filename + "\r\n");
                    str.Append("上传类型=项目" + "\r\n");
                    str.Append("上传文件大小=" + files.ContentLength+"\r\n");
                    str.Append("上传日期=" + vTime + "\r\n");
                    str.Append("上传登录名=" + user.登录名 + "\r\n");
                    str.Append("上传用户=" + user.用户姓名 + "\r\n");
                    str.Append("上传计算机=" + clientPCName + "\r\n");
                    str.Append("是否逻辑删除=0");
                    StreamWriter sw;
                    if (!System.IO.File.Exists(info))
                    {
                        sw = System.IO.File.CreateText(info);
                    }
                    else
                    {
                        sw = System.IO.File.AppendText(info);
                    }
